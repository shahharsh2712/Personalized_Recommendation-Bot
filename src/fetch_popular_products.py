...
def fetch_popular_products(cursor=None, batch_size=20):
    """Fetch popular products from Product Hunt API sorted by votes"""
    url = "https://api.producthunt.com/v2/api/graphql"

    # Create cursor parameter if needed
    cursor_param = f', after: "{cursor}"' if cursor else ""

    # GraphQL query to fetch posts (products) ordered by votes
    query = f"""
    {{
      posts(first: {batch_size}{cursor_param}, order: VOTES) {{
        pageInfo {{
          endCursor
          hasNextPage
        }}
...
def main():
    """Main function to fetch popular products from Product Hunt"""
    print("Fetching popular products from Product Hunt API...")
    ...
    has_next_page = True
    batch_size = 20
    target_count = 1000  # Increased target for popular products

    # Fetch products in batches
    while has_next_page and len(all_products) < target_count:
        print(
            f"Fetching batch of popular products (current count: {len(all_products)})"
        )

        # Fetch a batch of products
        products_data = fetch_popular_products(cursor, batch_size)
        ...
        print(
            f"Latest product votes: {processed_batch[-1]['votes_count'] if processed_batch else 'N/A'}"
        )

    print(f"Successfully collected {len(all_products)} popular products")

    # Save to JSON file
    output_file = "data/popular_products.json"
    ...
    for product in all_products[:3]:
        print(
            f"- {product['name']}: {product['tagline']} (Votes: {product['votes_count']})"
        )
